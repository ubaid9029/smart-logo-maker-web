...
export default function CaseStudiesPage() {
  return (
    <main className="relative flex min-h-[calc(100dvh-80px)] flex-col justify-between overflow-hidden bg-white">
      <div className="pointer-events-none absolute inset-y-0 left-0 right-0 mx-auto w-full max-w-7xl rounded-[1.75rem] bg-[radial-gradient(circle_at_top_left,rgba(15,23,42,0.05),transparent_30%),radial-gradient(circle_at_bottom_right,rgba(255,92,1,0.10),transparent_28%),linear-gradient(180deg,#ffffff_0%,#fffaf7_100%)]" />

      <section className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 pb-3 sm:pb-4 pt-20 sm:pt-22">
      ...
      <section className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 pb-3 sm:pb-4">
        <div className="grid grid-cols-1 gap-2.5 sm:gap-3 md:grid-cols-2">
          {caseStudies.map((item) => {
            ...
            return (
              <article
                key={item.title}
                className="rounded-[1.2rem] border border-white/70 bg-white/92 p-3.5 sm:p-4 shadow-[0_10px_28px_rgba(15,23,42,0.08)]"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-sm font-medium leading-6 text-slate-600">{item.summary}</p>
                    <div className="mt-3 rounded-[1rem] bg-slate-50 px-3 py-3">
                      <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Key takeaway</p>
                      <p className="mt-1 text-sm font-medium leading-5 text-slate-700">{item.takeaway}</p>
                    </div>
                  </div>
                </div>
              </article>
      ...
      <section className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 pb-0 sm:pb-1">
        <div className="rounded-[1.6rem] border border-white/70 bg-gradient-to-r from-slate-950 via-[#1f1037] to-[#4a1628] px-4 sm:px-6 py-5 sm:py-6 text-white shadow-[0_12px_35px_rgba(15,23,42,0.18)]">
