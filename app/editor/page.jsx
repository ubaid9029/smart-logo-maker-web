...
import { Palette, Type, Zap, ShoppingCart, LayoutGrid, X, Check } from 'lucide-react';
import { useSelector } from 'react-redux';
import { motion, AnimatePresence } from 'framer-motion';

const LogoCanvas = dynamic(() => import('@/components/Editor/Canvas'), { ssr: false });
...
function EditorUI() {
  ...
  const [logoConfig, setLogoConfig] = useState({
    imageUrl: searchParams.get('img') || '/photo1.jfif',
    text: searchParams.get('text') || formData?.name || 'BRAND',
    bgColor: '#FFFFFF',
    fontFamily: 'Arial',
    textColor: '#1A1A1A',
    iconPos: 'center' 
  ...
  const fonts = ['Arial', 'Verdana', 'Georgia', 'Impact', 'Courier New', 'serif'];
  const colors = [
    { val: '#FFFFFF' }, { val: '#E0F2FE' }, { val: '#DCFCE7' }, 
    { val: '#FFEDD5' }, { val: '#1A1A1A' }, { val: '#FCE7F3' }
  ];

  return (
    <div className="fixed inset-0 bg-[#f4f7fa] flex flex-col lg:flex-row overflow-hidden font-sans">
      <aside className="hidden lg:flex w-72 bg-white border-r flex-col p-6 shrink-0 overflow-y-auto">
        <h2 className={`text-xs font-black uppercase mb-6 ${gradients.text}`}>Variations</h2>
        <div className="space-y-4">
          {[1, 2, 3, 4].map((v) => (
            <div key={v} onClick={() => setLogoConfig({...logoConfig, bgColor: v % 2 === 0 ? '#1A1A1A' : '#FFFFFF'})} 
                 className="bg-white border p-4 rounded-2xl hover:border-orange-400 cursor-pointer transition-all shadow-sm">
              <div className="w-full h-20 bg-gray-50 rounded-xl mb-2 flex items-center justify-center">
                 <img src={logoConfig.imageUrl} className="w-10 h-10 object-contain" />
              </div>
              <span className="text-[10px] font-bold text-gray-400">APPLY STYLE {v}</span>
            </div>
          ))}
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-full relative">
        <header className="h-20 bg-white border-b px-8 flex items-center justify-end gap-3 shrink-0 z-50">
          <button onClick={() => setLogoConfig({...logoConfig, iconPos: logoConfig.iconPos === 'center' ? 'left' : 'center'})} 
                  className="flex items-center gap-2 px-5 py-2.5 border rounded-full text-sm font-bold hover:bg-gray-50 transition-all"><LayoutGrid size={16}/> Position</button>
          
          {/* Font Dropdown */}
          <div className="relative">
            <button onClick={() => setActiveDropdown(activeDropdown === 'font' ? null : 'font')} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-white text-sm font-bold shadow-md ${gradients.primary}`}><Type size={16}/> Font</button>
            {activeDropdown === 'font' && (
              <div className="absolute top-full right-0 mt-2 w-48 bg-white shadow-2xl rounded-2xl p-2 border">
                {fonts.map(f => <button key={f} onClick={() => {setLogoConfig({...logoConfig, fontFamily: f}); setActiveDropdown(null)}} className="block w-full p-2 text-sm hover:bg-gray-100">{f}</button>)}
              </div>
            )}
          </div>
          ...
          {/* Color Dropdown */}
          <div className="relative">
            <button onClick={() => setActiveDropdown(activeDropdown === 'color' ? null : 'color')} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-white text-sm font-bold shadow-md ${gradients.primary}`}><Palette size={16}/> Color</button>
            {activeDropdown === 'color' && (
              <div className="absolute top-full right-0 mt-2 w-48 bg-white shadow-2xl rounded-2xl p-4 grid grid-cols-3 gap-2">
                {colors.map(c => <button key={c.val} onClick={() => {setLogoConfig({...logoConfig, bgColor: c.val}); setActiveDropdown(null)}} className="w-10 h-10 rounded-lg border" style={{backgroundColor: c.val}} />)}
              </div>
            )}
          </div>
        </header>

        <section className="flex-1 flex items-center justify-center p-6 bg-[#f8fafc] overflow-hidden">
          <LogoCanvas config={logoConfig} />
        </section>

        <footer className="h-24 bg-white border-t flex items-center justify-center gap-4 px-6 shrink-0">
          <button onClick={() => setShowPreview(true)} className="px-10 py-3.5 border-2 rounded-full font-bold">Preview</button>
          <button className={`px-10 py-3.5 rounded-full font-bold text-white ${gradients.primary}`}>Save Design</button>
          <button className="flex items-center gap-2 px-10 py-3.5 bg-orange-500 text-white rounded-full font-bold"><ShoppingCart size={18}/> Buy Now</button>
        </footer>
      </main>
      ...
      <AnimatePresence>
        {showPreview && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4" onClick={() => setShowPreview(false)}>
            <motion.div initial={{scale:0.9}} animate={{scale:1}} exit={{scale:0.9}} className="bg-white p-8 rounded-[2.5rem] w-80 text-center" onClick={e => e.stopPropagation()}>
              <img src={logoConfig.imageUrl} className="w-20 h-20 mx-auto mb-4"/>
              <h3 className="font-bold text-xl mb-6">{logoConfig.text}</h3>
              <div className="flex gap-2"><button className="flex-1 bg-black text-white py-3 rounded-xl" onClick={() => setShowPreview(false)}>Edit</button><button className="flex-1 bg-orange-500 text-white py-3 rounded-xl">Buy</button></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
...
export default function EditorPage() { return <Suspense><EditorUI /></Suspense> }
