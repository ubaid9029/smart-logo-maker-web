...
const Fonts = ({ onNext, onBack, data, setData }) => {
  ...
  return (
    ...
    <div className="pt-20 flex flex-col items-center w-full animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-6xl  px-4 md:px-0 mx-auto ">
      ...
      {/* 1. Back Button - Removed fixed width w-4xl */}
      ...
      {/* 2. Title Section - Responsive Text Sizes */}
      <div className="flex flex-col items-center mb-8 md:mb-12 text-center">
        <div className="p-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] bg-white shadow-xl shadow-pink-100/50 mb-4 md:mb-6 border border-white">
          <Type size={32} className="text-pink-600 md:w-[40px] md:h-[40px]" />
        </div>
      ...
      {/* 3. Fonts Grid - Responsive Columns (1 on mobile, 2 on sm, 3 on md) */}
        ...
        {fontStyles.map((font) => {
          ...
          return (
            <button
              ...
              `}
            >
              {/* Font Preview - Adjusted for mobile */}
              <span className={`text-1xl md:text-5xl text-slate-900 mb-1 md:mb-2 ${font.style}`}>
                AaBbCc
              </span>
              ...
              {/* Selection Badge */}
              {isSelected && (
                <div className="absolute top-2 right-2 md:-top-2 md:-right-2 bg-gradient-to-r from-[#FF5C00] to-[#FF007A] p-1.5 rounded-full shadow-lg border-2 border-white animate-in zoom-in-50 duration-300">
      ...
      {/* 4. Action Buttons - Stack on mobile, side-by-side on desktop */}
