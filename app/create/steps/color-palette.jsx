...
const ColorPalette = ({ onBack, data, setData }) => {
  ...
  return (
    <div className=" pt-20 flex flex-col items-center w-full animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-6xl mx-auto px-4 md:px-6">
      ...
      {/* Back Button - Responsive max-width */}
      ...
      <div className="flex flex-col items-center mb-8 md:mb-12 text-center">
        <div className="p-4 md:p-5 rounded-[1.5rem] md:rounded-[2rem] bg-white shadow-xl shadow-pink-100/50 mb-4 md:mb-6 border border-white">
          <Palette size={32} className="text-pink-600 md:w-[40px] md:h-[40px]" />
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 w-full max-w-5xl mb-10 md:mb-12">
        {palettes.map((palette) => {
          ...
          return (
            <button
              ...
              className={`relative p-4 md:p-5 rounded-[1.8rem] md:rounded-[2.5rem] flex flex-col items-center justify-between gap-4 md:gap-5 transition-all duration-500 ease-out border-2 ${
                isSelected ? 'bg-white shadow-2xl border-pink-300 scale-[1.03] z-10' : 'bg-white/60 border-transparent hover:bg-white hover:shadow-md'
              }`}
            >
              <div className={`w-full h-24 md:h-32 rounded-[1.2rem] md:rounded-[1.5rem] bg-gradient-to-r ${palette.gradient} shadow-inner`}></div>
              ...
              {isSelected && (
                <div className="absolute top-2 right-2 md:-top-2 md:-right-2 bg-gradient-to-r from-[#FF5C00] to-[#FF007A] p-1.5 md:p-2 rounded-full shadow-lg border-2 border-white animate-in zoom-in-50 duration-300">
                  <Check size={14} className="text-white md:w-[16px]" strokeWidth={4} />
                </div>
              )}
            </button>
          );
      ...
      {/* Action Buttons - Mobile Stacked, Reverse Order for better UX */}
      <div className="flex flex-col-reverse md:flex-row items-center justify-center gap-4 md:gap-6 w-full max-w-2xl mb-10">
        ...
        <button
          ...
          className={`w-full md:w-1/2 py-5 md:py-6 rounded-2xl text-lg md:text-xl font-black text-white transition-all duration-500 shadow-xl flex items-center justify-center gap-3 ${
            data.color && !loading 
