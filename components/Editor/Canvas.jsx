...
export default function Canvas({ config }) {
  ...
  const iconX = config.iconPos === 'left' ? 80 : 250 - 60;

  return (
    <div className="bg-white shadow-2xl rounded-[2.5rem] p-4 border border-gray-100 overflow-hidden">
      <Stage width={500} height={350}>
        <Layer>
          <Rect width={500} height={350} fill={config.bgColor} cornerRadius={40} />
          
          {/* Watermark */}
          <Text text="Smart Logo Maker" x={0} y={320} width={500} fontSize={12} fill="#9CA3AF" align="center" opacity={0.5} />
          
          {img && <KonvaImage image={img} x={iconX} y={70} width={120} height={120} />}
          
          <Text text={config.text} y={220} width={500} fontSize={40} fontFamily={config.fontFamily} fill={config.bgColor === '#1A1A1A' ? '#FFFFFF' : '#1A1A1A'} align="center" fontStyle="bold" />
        </Layer>
      </Stage>
    </div>
  );
